...
class RanklistHourEntranceMessage(proto.Message):
    """小时榜入口消息（字段结构待审计，仅定义 Common 用于分发）。"""
    common = proto.Field(Common, number=1)


class RoomDataSyncMessage(proto.Message):
    """房间数据同步消息（字段结构待审计，仅定义 Common 用于分发）。"""
    common = proto.Field(Common, number=1)


class LightGiftInfo(proto.Message):
    """轻礼物的礼物详情（ID + 图片 + 单礼物抖币价值）。

    与 GiftStruct 不同，轻礼物不含名称/连击/面板等完整字段，
    钻石价值直接在此结构体的 diamond_count 中，无需查表。
    """
    gift_id = proto.Field(proto.UINT64, number=1)
    image = proto.Field(Image, number=2)
    diamond_count = proto.Field(proto.UINT64, number=3)


class LightGiftMessage(proto.Message):
    """轻礼物消息（快捷礼物、粉丝团礼物、人气票等）。

    与 GiftMessage 的区别：
    - 不含完整 User 对象，仅 user_id 字段（field 5）
    - 不含 combo_count / repeat_count 等连击字段
    - 礼物价值直接在 gift_info.diamond_count 中
    - count 字段（field 10）表示本次赠送数量
    """
    common = proto.Field(Common, number=1)
    msg_type = proto.Field(proto.UINT64, number=2)
    version = proto.Field(proto.UINT64, number=3)
    user_id = proto.Field(proto.UINT64, number=5)
    gift_info = proto.Field(LightGiftInfo, number=7)
    unknown_8 = proto.Field(proto.BYTES, number=8)
    count = proto.Field(proto.UINT64, number=10)
    unknown_12 = proto.Field(proto.UINT64, number=12)


class ProfitInteractionScoreMessage(proto.Message):
    """收益互动分数消息（字段结构待审计，仅定义 Common 用于分发）。"""
    common = proto.Field(Common, number=1)
...
class LinkmicBattleMessage(proto.Message):
    """PK 战斗消息（开始/状态更新）。"""
    common = proto.Field(Common, number=1)


class LinkmicBattleFinishMessage(proto.Message):
    """PK 战斗结束消息。"""
    common = proto.Field(Common, number=1)


class LinkmicArmiesMessage(proto.Message):
    """PK 阵营信息。"""
    common = proto.Field(Common, number=1)


class LinkmicPlayModeUpdateScoreMessage(proto.Message):
    """PK 玩法模式分数更新。"""
    common = proto.Field(Common, number=1)


class LinkerContributeMessage(proto.Message):
    """PK 贡献信息。"""
    common = proto.Field(Common, number=1)


class BattleAuxiliaryMessage(proto.Message):
    """战斗辅助消息。"""
    common = proto.Field(Common, number=1)


class BattleEndPunishMessage(proto.Message):
    """战斗结束惩罚消息。"""
    common = proto.Field(Common, number=1)


class BattlePowerContainerMessage(proto.Message):
    """战斗力容器消息。"""
    common = proto.Field(Common, number=1)
